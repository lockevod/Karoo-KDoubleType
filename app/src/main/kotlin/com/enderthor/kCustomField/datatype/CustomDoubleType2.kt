        ...
        val job = CoroutineScope(Dispatchers.IO).launch {
            ...
            context.streamSettings()
                ...
                .collect { (settings, generalSettings) ->
                    karooSystem.streamDataFlow(settings.customleft2.action)
                        .combine(karooSystem.streamDataFlow(settings.customright2.action)) { left: StreamState, right: StreamState -> Quadruple(generalSettings, settings,left, right) }
                        .collect { (generalsettings, settings, left: StreamState, right: StreamState) ->
                            ...
                            val colorleft= if(settings.customleft2zone == true) ColorFilter.tint(ColorProvider(Color.Black, Color.Black)) else ColorFilter.tint(ColorProvider(day = Color(ContextCompat.getColor(context, settings.customleft2.colorday)), night = Color(ContextCompat.getColor(context, settings.customleft2.colornight))))
                            val colorright= if(settings.customright2zone == true) ColorFilter.tint(ColorProvider(Color.Black, Color.Black)) else ColorFilter.tint(ColorProvider(day = Color(ContextCompat.getColor(context,settings.customright2.colorday)), night = Color(ContextCompat.getColor(context,settings.customright2.colornight))))

                            val colorzoneleft = ColorProvider(
                                day = Color(ContextCompat.getColor(context, getZone(
                                    if (settings.customleft2.zone == "heartRateZones") userProfile.heartRateZones else if (settings.customleft2.zone == "powerZones") userProfile.powerZones else slopeZones,
                                    leftValue
                            ...
                            ).takeIf { settings.customleft2zone == true } ?: ColorProvider(Color.White, Color.Black)

                            val colorzoneright = ColorProvider(
                                day = Color(ContextCompat.getColor(context, getZone(
                                    if (settings.customright2.zone == "heartRateZones") userProfile.heartRateZones else if (settings.customright2.zone == "powerZones") userProfile.powerZones else slopeZones,
                                    rightValue
                                )?.let { if (generalsettings.ispalettezwift) it.colorZwift else it.colorResource} ?: R.color.zone7)),
                                night = Color(ContextCompat.getColor(context, getZone(
                                    if (settings.customright2.zone == "heartRateZones") userProfile.heartRateZones else if (settings.customright2.zone == "powerZones") userProfile.powerZones else slopeZones,
                            ...
                            ).takeIf { settings.customright2zone == true } ?: ColorProvider(Color.White, Color.Black)


                            //Timber.d("Updating view ($emitter) with $leftValue and $rightValue")
                            val result = glance.compose(context, DpSize.Unspecified) {
                                DoubleTypesScreen(leftValue.roundToInt(), rightValue.roundToInt(), settings.customleft2.icon, settings.customright2.icon,colorleft,colorright,settings.isvertical2,colorzoneleft,colorzoneright,config.gridSize.second > 18,karooSystem.hardwareType == HardwareType.KAROO,generalsettings.iscenteralign)
                            }
